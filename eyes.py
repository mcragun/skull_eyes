...
MOUSE           = True  #Mouse support to blink eyes
MOUSE_BUTTONS   = [0,0]
GPAD            = True
# GPIO initialization ------------------------------------------------------
...
#GAMEPAD STUFF
#gpad_arr = LX, LY, RX, RY, LTrig, RTrig
gpadValue = [0, 0, 0, 0, 0, 0]
...
def gPadThread(gamepad, dest):
    while True:
        for event in gamepad.read_loop():
            if event.type == 3:
                if event.code == 0:
                    name = "Left X-axis"
                    #Get the value, and normalize it from 0,1 then flip it
                    dest[0] = -1*event.value/255.0 + 1
                if event.code == 1:
                    name = "Left Y-axis"
                    dest[1] = -1*int(event.value)/255.0 + 1
                if event.code == 5:
                    name = "Right Y-axis"
                    dest[3] = -1*int(event.value)/255.0 +1
                if event.code == 2:
                    name = "Right X-axis"
                    dest[2] = -1*int(event.value)/255.0 +1
            elif event.type == 1:
                if event.code == 318:
                    name = "Left Trigger"
                    if event.value == 1:
                        dest[4] = int(event.value)
                if event.code == 317:
                    name = "Right Trigger"
                    if event.value == 1:
                        dest[5] = int(event.value)
            print dest

if MOUSE:
    thread.start_new_thread(mouseThread, (MOUSE_BUTTONS,))
# Start ADC sampling thread if needed:
if adc:
    thread.start_new_thread(adcThread, (adc, adcValue))

if GPAD:
    thread.start_new_thread(gPadThread, (gamepad, gpadValue))
	...
	if JOYSTICK_X_IN >= 0 and JOYSTICK_Y_IN >= 0:
        ...
        if CRAZY_EYES:
            if GPAD:
                curXR = gpadValue[0]
                curYR = gpadValue[1]
                curXR = -45.0 + curXR * 90.0
                curYR = -30.0 + curYR * 60.0
